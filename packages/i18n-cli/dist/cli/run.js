...
var i18nTypes = require('@gravity-ui/i18n-types');
var fs2 = require('fs/promises');
var path = require('path');
require('child_process');
var estreeToolkit = require('estree-toolkit');
var astring = require('astring');
var parseIcuMessage = require('format-message-parse');
var createPrettyIcuMessage = require('format-message-print');
...
var parseIcuMessage__default = /*#__PURE__*/_interopDefault(parseIcuMessage);
var createPrettyIcuMessage__default = /*#__PURE__*/_interopDefault(createPrettyIcuMessage);
...
var toAbsolutePath = (path2) => path.resolve(appRootPath, toRelativePath(path2));
...
var createRelativeImport = (fromPath, toPath, fromAlias) => {
  ...
  const parsedToAbsPath = path.parse(toPath);
  if (fromAlias && path.parse(toAbsPath).dir !== path.parse(fromAbsPath).dir) {
    return fromAlias;
  }
  let result = path.relative(parsedToAbsPath.ext ? parsedToAbsPath.dir : toAbsPath, fromAbsPath);
...
var loadProjectConfig = (searchPlaces) => {
  ...
  const explorer = cosmiconfig.cosmiconfigSync(MODULE_NAME, {
    cache: false,
...
function prettifyMultilineICU(message) {
  try {
    const ast = parseIcuMessage__default.default(message);
    return createPrettyIcuMessage__default.default(ast);
  } catch (err) {
...
function generateMessageLiteral(message) {
  if (message === void 0) {
    return estreeToolkit.builders.identifier("undefined");
  }
  if (message === null) {
  ...
  if (isMessageLikeMultilineICU(message)) {
    ...
    return estreeToolkit.builders.templateLiteral(
      [estreeToolkit.builders.templateElement({ raw: prettyICU, cooked: prettyICU }, true)],
      []
    );
  }
  return estreeToolkit.builders.literal(message);
}
function generateLegacyPluralObject(value) {
  return estreeToolkit.builders.objectExpression(
    Object.entries(value).map(
      ([form, formValue]) => estreeToolkit.builders.property(
        "init",
        estreeToolkit.builders.identifier(form),
...
function generateMeta(meta) {
  const properties = [];
  if (meta.id) {
    properties.push(estreeToolkit.builders.property("init", estreeToolkit.builders.identifier("id"), estreeToolkit.builders.literal(meta.id)));
  }
  if (meta.description) {
    properties.push(
      estreeToolkit.builders.property("init", estreeToolkit.builders.identifier("description"), estreeToolkit.builders.literal(meta.description))
    );
  }
  if (typeof meta.markdown === "boolean") {
    properties.push(estreeToolkit.builders.property("init", estreeToolkit.builders.identifier("markdown"), estreeToolkit.builders.literal(meta.markdown)));
  }
  return properties.length ? estreeToolkit.builders.objectExpression(properties) : void 0;
}
function generateMessageObject(message, allowedLocales) {
  ...
  for (const [key, val] of Object.entries(message)) {
    ...
    if (key === "meta") {
      ...
      if (metaObj) {
        metaProp = estreeToolkit.builders.property("init", estreeToolkit.builders.literal("meta"), metaObj);
      }
    } else if (i18nTypes.isLegacyPlural(val)) {
      node = generateLegacyPluralObject(val);
    } else if (typeof val === "string" || val === null) {
      node = generateMessageLiteral(val);
    }
    if (node) {
      propsRecord[key] = estreeToolkit.builders.property("init", estreeToolkit.builders.literal(key), node);
    }
  }
  const sortedProps = allowedLocales.map(
    (locale) => propsRecord[locale] || estreeToolkit.builders.property("init", estreeToolkit.builders.literal(locale), estreeToolkit.builders.literal(null))
  );
  if (metaProp) {
    sortedProps.push(metaProp);
  }
  return estreeToolkit.builders.objectExpression(sortedProps);
}
function generateMessagesObject(messages, allowedLocales) {
  ...
  return estreeToolkit.builders.objectExpression(
    sortedMessages.map(
      ({ message, meta }) => estreeToolkit.builders.property(
        "init",
        estreeToolkit.builders.literal(meta.objectKey),
...
function generateTranslationsFileContent(params) {
  ...
  const content = astring.generate(
    estreeToolkit.builders.program(
      [
        estreeToolkit.builders.exportNamedDeclaration(
          estreeToolkit.builders.variableDeclaration("const", [
            estreeToolkit.builders.variableDeclarator(
              estreeToolkit.builders.objectPattern(
                intlModule === config.serverIntlModule ? [
                  estreeToolkit.builders.property(
                    "init",
                    estreeToolkit.builders.identifier("messages"),
                    estreeToolkit.builders.identifier("messages"),
                    false,
                    true
                  )
                  ...
                  estreeToolkit.builders.property(
                    "init",
                    estreeToolkit.builders.identifier(DEFAULT_EXPORT_ALIASES.t),
                    estreeToolkit.builders.identifier(tExportName),
                    false,
                    tExportName === DEFAULT_EXPORT_ALIASES.t
                  ),
                  estreeToolkit.builders.property(
                    "init",
                    estreeToolkit.builders.identifier(DEFAULT_EXPORT_ALIASES.Message),
                    estreeToolkit.builders.identifier(messageExportName),
                    false,
                    messageExportName === DEFAULT_EXPORT_ALIASES.Message
                  )
                ]
              ),
              estreeToolkit.builders.callExpression(
                estreeToolkit.builders.memberExpression(
                  estreeToolkit.builders.identifier("intl"),
                  estreeToolkit.builders.identifier("createMessages")
                ),
...
function generateOutputFilename(outputPath) {
  ...
  return path.resolve(parsedPath.ext ? parsedPath.dir : absPath, TRANSLATIONS_FILE_POSTFIX);
}
async function generateTranslationsFile(params) {
  ...
  try {
    ...
    await fs2.writeFile(translationsFilePath, prettyCode);
  } catch (err) {
...
async function parseToAst({ filename, content }) {
  return parseToAstSync({
    filename,
    content: content ?? await fs.promises.readFile(filename, { encoding: "utf-8" })
  });
}
...
function isCreateMessagesCall(node) {
  return node.type === types.AST_NODE_TYPES.CallExpression && node.callee.type === "MemberExpression" && node.callee.property.type === "Identifier" && node.callee.property.name === "createMessages";
}
...
function parseMessage(node) {
  ...
  const { pluralCount, normalCount } = Object.entries(message).reduce(
    (acc, [key, cur]) => {
      ...
      const isPlural = i18nTypes.isLegacyPlural(cur);
      return {
  ...
  typescriptEstree.simpleTraverse(ast, {
    visitors: {
      VariableDeclarator: (node) => {
        if (node.type === types.AST_NODE_TYPES.VariableDeclarator && node.id.type === types.AST_NODE_TYPES.ObjectPattern && node.init && node.init.type === types.AST_NODE_TYPES.CallExpression && isCreateMessagesCall(node.init)) {
          node.id.properties.forEach((prop) => {
            if (prop.type === types.AST_NODE_TYPES.Property && prop.key.type === types.AST_NODE_TYPES.Identifier) {
              ...
              if (prop.computed === false && prop.value.type === types.AST_NODE_TYPES.Identifier && originalName !== prop.value.name && isTranslationFunction(originalName)) {
      ...
      CallExpression: (node) => {
        if (node && node.type === types.AST_NODE_TYPES.CallExpression && isCreateMessagesCall(node)) {
...
var throughDirs = async (moveFrom, recursively, dirCallback) => {
  const direntList = await fs2__default.default.readdir(moveFrom, { withFileTypes: true });
  const getTsFiles = () => {
  ...
  const pList = direntList.reduce((acc, dirent) => {
    const fullPath = path__default.default.resolve(moveFrom, dirent.name);
...
var addImportLine = async (filePath, i18nCalleeUsed, i18nPath) => {
  ...
  await fs2.writeFile(filePath, `${importLine}
${code}`);
};
...
var job = async (filePath, localesList, fallbackLocales) => {
  ...
  const getI18nSourcePath = (i18nImport) => {
    ...
    if (i18nImport.startsWith("./") || i18nImport.startsWith("../")) {
      return `${path.join(dirPath, i18nImport)}.ts`;
    }
    return void 0;
  };
...
var runCreateKeys = async (argv, projectConfig) => {
  ...
  const isFilePathDir = fs.lstatSync(FILE_PATH).isDirectory();
  if (isFilePathDir) {
    const filePathList = fs.readdirSync(FILE_PATH, { withFileTypes: true }).reduce(
      (acc, file) => {
...
var runFindUnused = async (argv) => {
  const isFilePathDir = fs.lstatSync(argv.path).isDirectory();
...
var run = async () => {
  ...
  const argv = helpers.hideBin(process.argv);
  const { name, version } = pkg;
  const parsedArgs = await yargs__default.default.wrap(120).version(version).option("config", {
    alias: "c",
    type: "string",
  ...
  const args = yargs__default.default(argv);
    ...
    args.command(
      ...
      async (args2) => {
        try {
          ...
          console.log("");
          console.error(picocolors.red(stack || message));
          process.exit(1);
        }
      }
    );
