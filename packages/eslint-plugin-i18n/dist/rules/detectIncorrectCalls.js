...
function isI18nBind(node, globalI18nName) {
...
function isI18nDeclaration(node, globalI18nName) {
...
function getI18nDeclarationParams(node) {
...
function create(context) {
    const { globalI18nName = DEFAULT_GLOBAL_I18N_NAME, variableI18nPrefix = DEFAULT_I18N_PREFIX, globalI18nHelpers = DEFAULT_GLOBAL_I18N_HELPERS, ignoredIdentifiers: ignoredIdentifiersOrig = DEFAULT_IGNORED_IDENTIFIERS, } = context.options[0] || {};
    ...
    return {
        ...
        CallExpression(node) {
            if (isI18nBind(node, globalI18nName) &&
                node.parent &&
            ...
            if (node.callee.name === globalI18nName) {
                ...
                if (params && !isValidParams(params)) {
                    context.report({
                        node: params,
                        message: 'Parameters must be object like {someParam: 123}, but get {{ type }}',
                        data: {
                            type: params.type,
                        },
                    });
            ...
            if (isI18nIdentifier) {
                ...
                context.report({
                    node,
                    message: '{{ variable }} must be declared in the current file, but declaration not found',
                    data: {
        ...
        ExportDefaultDeclaration(node) {
            if (isI18nBind(node.declaration, globalI18nName) ||
...
exports.rule = {
    ...
    meta: {
        type: 'problem',
        docs: {
            description: 'Detects calls like "i18n(dynamicVariable)", which don\'t use a string literal as a key.\nCheck that all used i18n identifiers are declared in the current file (except global).',
            recommended: true,
...
//# sourceMappingURL=detectIncorrectCalls.js.map