...
import { createTranslationsFileVisitor, } from '@gravity-ui/i18n-babel-plugin';
...
export async function transformFileWithBabel(code, options) {
...
export function includeDynamicImportToTranslationsFile(code, languagesDirName = LANGUAGES_DIR_PREFIX) {
    ...
    if (createMessagesIndex !== -1 && exportConstIndex !== -1) {
        s.prependLeft(exportConstIndex, `const langs = require.context("./${languagesDirName}/", false, /\\/.*\\.ts$/);\nconst messages = langs("./" + ${LANGUAGE_VARIABLE_NAME_ENV_KEY} + ".ts").default;\n`);
...
async function transformTranslationsByLocale(code, filePath, options) {
...
export async function transformTranslationFile(code, filePath, options) {
...
//# sourceMappingURL=transformTranslationFile.js.map